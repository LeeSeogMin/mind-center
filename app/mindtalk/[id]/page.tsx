    ...
    return (
      <div className="bg-[#F0FAF3] min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-[#1E3A26] font-medium mb-2">글을 찾을 수 없습니다.</p>
          <Link href="/mindtalk" className="text-sm text-[#4A8C5E] hover:underline">
            목록으로 돌아가기
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="bg-[#F0FAF3] min-h-screen">
      <div className="max-w-[800px] mx-auto px-6 py-12">
        <Link href="/mindtalk" className="inline-flex items-center gap-2 text-sm text-[#6B8C7B] hover:text-[#4A8C5E] mb-6">
          <ArrowLeft className="w-4 h-4" /> 목록으로 돌아가기
        </Link>

        {/* Post */}
        <Card className="border-[#D0E8D8] rounded-2xl mb-6">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="font-heading text-2xl">{post.title}</CardTitle>
              <Badge className={post.is_answered ? "bg-[#4A8C5E]" : "border-[#D0E8D8] text-[#6B8C7B]"}>
                ...
                <div className="flex gap-2">
                  ...
                  <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                    ...
                    <DialogContent>
                      ...
                      <DialogFooter>
                        <Button variant="outline" onClick={() => setDeleteDialogOpen(false)} className="border-[#D0E8D8]">
                          취소
                        </Button>
        ...
        <div className="space-y-4">
          ...
          {comments.length === 0 && (
            <p className="text-sm text-[#6B8C7B] py-4">아직 답변이 없습니다.</p>
          )}
        ...
        {!user && (
          <>
            ...
            <div className="text-center py-4">
              <p className="text-sm text-[#6B8C7B] mb-2">답글을 작성하려면 로그인이 필요합니다.</p>
              <Link href="/login">
