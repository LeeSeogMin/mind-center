...
```
홈 → "마음톡" 진입 → "글쓰기" 클릭 → 로그인 페이지로 이동 → Google 로그인
...
Copilot은 **이미지를 이해하는 기능**(Copilot Vision)을 제공한다. Copilot Chat에 이미지를 첨부하면, AI가 그림을 분석하여 코드를 생성한다.
...
> [!NOTE]
> ### 💡 v0.dev 무료 이용 가이드 (Free 플랜)
> 
> **1. 기본 혜택**
> - **매월 $5 크레딧** 지급 (월 초 리셋)
> - **하루 최대 7개 메시지** 전송 가능
> - 모델 선택 가능 (v0 Mini / Pro / Max / Max Fast)
> 
> **2. 주요 무료 기능**
> - **텍스트 UI 생성**: 프롬프트로 React/Next.js 코드 제작
> - **이미지 업로드**: 손그림 스케치, 스크린샷으로 UI 생성 (Upload from computer)
> - **GitHub 연동**: 코드 불러오기 및 바로 push 가능 (Import from GitHub)
> - **Vercel 배포**: 제작한 앱을 즉시 웹에 배포
> - **디자인 모드**: 생성된 UI를 클릭해서 비주얼 편집 가능
> - **기타**: 디자인 시스템, 커스텀 지시사항(Instructions), MCP 연동, AI 이미지 생성(Generate Images) 등
> 
> **3. 크레딧 추가 팁 (Referral)**
> - 우측 상단 `Refer` 버튼을 통해 친구 초대 시 친구와 본인 모두 **$5 추가 크레딧** 증정
> - 친구가 유동 구독 시 **$20 크레딧** 추가 증정 (월 최대 $200)
> 
> **4. 학생을 위한 혜택**
> - 학교 이메일 인증 시 **v0 Premium 1년 무료** (Students 프로그램)
> 
> ※ Figma 직접 임포트(Create from Figma) 기능만 유료(Premium) 전용입니다.
